/*
generated at Sat May 18 14:38:22 2019
...
/* 38 */	"expr: sexpr",
/* 39 */	"expr: rexpr",
/* 40 */	"expr: CALL(ID,arguments)",
/* 41 */	"expr: CALL(ID,VOID)",
/* 42 */	"lvec: lval",
/* 43 */	"lval: ilval",
/* 44 */	"lval: rlval",
/* 45 */	"lval: slval",
/* 46 */	"ilval: LOCAL",
/* 47 */	"ilval: ID",
/* 48 */	"ilval: INDEX(lvec,expr)",
/* 49 */	"iassign: iexpr",
/* 50 */	"iassign: rexpr",
/* 51 */	"iexpr: INT",
/* 52 */	"iexpr: ATR(iassign,ilval)",
/* 53 */	"iexpr: ADD(iexpr,iexpr)",
/* 54 */	"iexpr: SUB(iexpr,iexpr)",
/* 55 */	"iexpr: DIV(iexpr,iexpr)",
/* 56 */	"iexpr: MOD(iexpr,iexpr)",
/* 57 */	"iexpr: MUL(iexpr,iexpr)",
/* 58 */	"iexpr: AND(iexpr,iexpr)",
/* 59 */	"iexpr: OR(iexpr,iexpr)",
/* 60 */	"iexpr: FACTORIAL(iexpr)",
/* 61 */	"iexpr: NOT(iexpr)",
/* 62 */	"iexpr: UMINUS(iexpr)",
/* 63 */	"iexpr: INCR(ilval)",
/* 64 */	"iexpr: DECR(ilval)",
/* 65 */	"iexpr: POSINC(ilval)",
/* 66 */	"iexpr: POSDEC(ilval)",
/* 67 */	"iexpr: REF(lval)",
/* 68 */	"iexpr: PTR(ilval)",
/* 69 */	"iexpr: PTR(PTR(ilval))",
/* 70 */	"iexpr: PTR(PTR(slval))",
/* 71 */	"iexpr: CALL(ID,arguments)",
/* 72 */	"iexpr: CALL(ID,VOID)",
/* 73 */	"cmp: iexpr",
/* 74 */	"cmp: sexpr",
/* 75 */	"iexpr: LT(cmp,cmp)",
/* 76 */	"iexpr: GT(cmp,cmp)",
...
/* 78 */	"iexpr: LE(cmp,cmp)",
/* 79 */	"iexpr: NE(cmp,cmp)",
/* 80 */	"iexpr: LOGIC_EQUAL(cmp,cmp)",
/* 81 */	"iexpr: AND(cmp,cmp)",
/* 82 */	"iexpr: OR(cmp,cmp)",
/* 83 */	"iexpr: LT(rexpr,rexpr)",
/* 84 */	"iexpr: GT(rexpr,rexpr)",
/* 85 */	"iexpr: GE(rexpr,rexpr)",
/* 86 */	"iexpr: LE(rexpr,rexpr)",
/* 87 */	"iexpr: LOGIC_EQUAL(rexpr,rexpr)",
/* 88 */	"iexpr: NE(rexpr,rexpr)",
/* 89 */	"slval: LOCAL",
/* 90 */	"slval: ID",
/* 91 */	"slval: INDEX(lvec,expr)",
/* 92 */	"sexpr: STR",
/* 93 */	"sexpr: PTR(slval)",
/* 94 */	"sexpr: PTR(PTR(ilval))",
/* 95 */	"sassign: sexpr",
/* 96 */	"sassign: iexpr",
/* 97 */	"sexpr: ATR(sassign,slval)",
/* 98 */	"sexpr: CALL(ID,arguments)",
/* 99 */	"sexpr: CALL(ID,VOID)",
/* 100 */	"rexpr: PTR(rlval)",
/* 101 */	"rexpr: PTR(PTR(ilval))",
/* 102 */	"rlval: LOCAL",
/* 103 */	"rlval: ID",
/* 104 */	"rlval: INDEX(lvec,expr)",
/* 105 */	"rexpr: ATR(rassign,rlval)",
/* 106 */	"rassign: rexpr",
/* 107 */	"rexpr: REAL",
/* 108 */	"rexpr: ADD(rexpr,rexpr)",
/* 109 */	"rexpr: UMINUS(rexpr)",
/* 110 */	"rexpr: SUB(rexpr,rexpr)",
/* 111 */	"rexpr: MUL(rexpr,rexpr)",
/* 112 */	"rexpr: DIV(rexpr,rexpr)",
/* 113 */	"rexpr: iexpr",
/* 114 */	"rexpr: CALL(ID,arguments)",
/* 115 */	"rexpr: CALL(ID,VOID)",
/* 116 */	"arguments: ARGS(NIL,expr)",
/* 117 */	"arguments: ARGS(arguments,expr)",
};
...
static void yyclosure_sexpr(NODEPTR_TYPE a, int c) {
	struct yystate *p = (struct yystate *)STATE_LABEL(a);
	yytrace(a, 95, c + 1, p->cost[yysassign_NT]);
	if (c + 1 < p->cost[yysassign_NT]) {
		p->cost[yysassign_NT] = c + 1;
		p->rule.yysassign = 1;
	}
	yytrace(a, 74, c + 1, p->cost[yycmp_NT]);
	if (c + 1 < p->cost[yycmp_NT]) {
		p->cost[yycmp_NT] = c + 1;
		p->rule.yycmp = 2;
	}
...
static void yyclosure_rexpr(NODEPTR_TYPE a, int c) {
	struct yystate *p = (struct yystate *)STATE_LABEL(a);
	yytrace(a, 106, c + 1, p->cost[yyrassign_NT]);
	if (c + 1 < p->cost[yyrassign_NT]) {
		p->cost[yyrassign_NT] = c + 1;
		p->rule.yyrassign = 1;
	}
	yytrace(a, 50, c + 1, p->cost[yyiassign_NT]);
	if (c + 1 < p->cost[yyiassign_NT]) {
		p->cost[yyiassign_NT] = c + 1;
		p->rule.yyiassign = 2;
	}
...
static void yyclosure_lval(NODEPTR_TYPE a, int c) {
	struct yystate *p = (struct yystate *)STATE_LABEL(a);
	yytrace(a, 42, c + 1, p->cost[yylvec_NT]);
...
static void yyclosure_slval(NODEPTR_TYPE a, int c) {
	struct yystate *p = (struct yystate *)STATE_LABEL(a);
	yytrace(a, 45, c + 0, p->cost[yylval_NT]);
	if (c + 0 < p->cost[yylval_NT]) {
		p->cost[yylval_NT] = c + 0;
		p->rule.yylval = 3;
		yyclosure_lval(a, c + 0);
...
static void yylabel(NODEPTR_TYPE a, NODEPTR_TYPE u) {
	...
	case 91: /* INDEX */
		...
		yytrace(a, 48, c + 0, p->cost[yyilval_NT]);
		...
		yytrace(a, 91, c + 0, p->cost[yyslval_NT]);
		...
		yytrace(a, 104, c + 0, p->cost[yyrlval_NT]);
	...
	case 257: /* INT */
		/* iexpr: INT */
		yytrace(a, 51, 1 + 0, p->cost[yyiexpr_NT]);
	...
	case 258: /* REAL */
		/* rexpr: REAL */
		yytrace(a, 107, 1 + 0, p->cost[yyrexpr_NT]);
	...
	case 259: /* ID */
		...
		/* slval: ID */
		c = (isString(a));
		yytrace(a, 90, c + 0, p->cost[yyslval_NT]);
		if (c + 0 < p->cost[yyslval_NT]) {
			p->cost[yyslval_NT] = c + 0;
			p->rule.yyslval = 2;
			yyclosure_slval(a, c + 0);
		...
		/* rlval: ID */
		c = (isReal(a));
		yytrace(a, 103, c + 0, p->cost[yyrlval_NT]);
		if (c + 0 < p->cost[yyrlval_NT]) {
			p->cost[yyrlval_NT] = c + 0;
			p->rule.yyrlval = 2;
			yyclosure_rlval(a, c + 0);
	...
	case 260: /* STR */
		/* sexpr: STR */
		yytrace(a, 92, 1 + 0, p->cost[yysexpr_NT]);
	...
	case 282: /* ATR */
		...
		yytrace(a, 97, c + 0, p->cost[yysexpr_NT]);
	...
	case 289: /* LOCAL */
		...
		/* slval: LOCAL */
		c = (isString(a));
		yytrace(a, 89, c + 0, p->cost[yyslval_NT]);
		if (c + 0 < p->cost[yyslval_NT]) {
			p->cost[yyslval_NT] = c + 0;
			p->rule.yyslval = 1;
			yyclosure_slval(a, c + 0);
		...
		/* rlval: LOCAL */
		c = (isReal(a));
		yytrace(a, 102, c + 0, p->cost[yyrlval_NT]);
		if (c + 0 < p->cost[yyrlval_NT]) {
			p->cost[yyrlval_NT] = c + 0;
			p->rule.yyrlval = 1;
			yyclosure_rlval(a, c + 0);
	...
	case 292: /* PTR */
		...
		yytrace(a, 93, c + 0, p->cost[yysexpr_NT]);
		if (c + 0 < p->cost[yysexpr_NT]) {
			p->cost[yysexpr_NT] = c + 0;
			p->rule.yysexpr = 2;
			yyclosure_sexpr(a, c + 0);
		...
		yytrace(a, 100, c + 0, p->cost[yyrexpr_NT]);
		if (c + 0 < p->cost[yyrexpr_NT]) {
			p->cost[yyrexpr_NT] = c + 0;
			p->rule.yyrexpr = 1;
			yyclosure_rexpr(a, c + 0);
		...
		if (	/* rexpr: PTR(PTR(ilval)) */
			...
			yytrace(a, 101, c + 0, p->cost[yyrexpr_NT]);
			if (c + 0 < p->cost[yyrexpr_NT]) {
				p->cost[yyrexpr_NT] = c + 0;
				p->rule.yyrexpr = 2;
				yyclosure_rexpr(a, c + 0);
	...
	case 293: /* CALL */
		...
		if (	/* sexpr: CALL(ID,arguments) */
			...
			yytrace(a, 98, c + 0, p->cost[yysexpr_NT]);
			if (c + 0 < p->cost[yysexpr_NT]) {
				p->cost[yysexpr_NT] = c + 0;
				p->rule.yysexpr = 5;
				yyclosure_sexpr(a, c + 0);
		...
		if (	/* sexpr: CALL(ID,VOID) */
			...
			c = (isString(a));
			yytrace(a, 99, c + 0, p->cost[yysexpr_NT]);
			if (c + 0 < p->cost[yysexpr_NT]) {
				p->cost[yysexpr_NT] = c + 0;
				p->rule.yysexpr = 6;
				yyclosure_sexpr(a, c + 0);
		...
		if (	/* rexpr: CALL(ID,VOID) */
			...
			c = (isReal(a));
			yytrace(a, 115, c + 0, p->cost[yyrexpr_NT]);
			if (c + 0 < p->cost[yyrexpr_NT]) {
				p->cost[yyrexpr_NT] = c + 0;
				p->rule.yyrexpr = 12;
				yyclosure_rexpr(a, c + 0);
	...
	case 298: /* ARGS */
		...
		yytrace(a, 117, c + 0, p->cost[yyarguments_NT]);
		if (c + 0 < p->cost[yyarguments_NT]) {
			p->cost[yyarguments_NT] = c + 0;
...
static void yykids(NODEPTR_TYPE p, int eruleno, NODEPTR_TYPE kids[]) {
	...
	switch (eruleno) {
	case 117: /* arguments: ARGS(arguments,expr) */
	case 112: /* rexpr: DIV(rexpr,rexpr) */
	case 111: /* rexpr: MUL(rexpr,rexpr) */
	case 110: /* rexpr: SUB(rexpr,rexpr) */
	case 108: /* rexpr: ADD(rexpr,rexpr) */
	case 105: /* rexpr: ATR(rassign,rlval) */
	case 104: /* rlval: INDEX(lvec,expr) */
	case 97: /* sexpr: ATR(sassign,slval) */
	case 91: /* slval: INDEX(lvec,expr) */
	case 88: /* iexpr: NE(rexpr,rexpr) */
	case 87: /* iexpr: LOGIC_EQUAL(rexpr,rexpr) */
	case 86: /* iexpr: LE(rexpr,rexpr) */
	case 85: /* iexpr: GE(rexpr,rexpr) */
	case 84: /* iexpr: GT(rexpr,rexpr) */
	case 83: /* iexpr: LT(rexpr,rexpr) */
	case 82: /* iexpr: OR(cmp,cmp) */
	case 81: /* iexpr: AND(cmp,cmp) */
	case 80: /* iexpr: LOGIC_EQUAL(cmp,cmp) */
	case 79: /* iexpr: NE(cmp,cmp) */
	case 78: /* iexpr: LE(cmp,cmp) */
	case 77: /* iexpr: GE(cmp,cmp) */
	case 76: /* iexpr: GT(cmp,cmp) */
	case 75: /* iexpr: LT(cmp,cmp) */
	case 59: /* iexpr: OR(iexpr,iexpr) */
	case 58: /* iexpr: AND(iexpr,iexpr) */
	case 57: /* iexpr: MUL(iexpr,iexpr) */
	case 56: /* iexpr: MOD(iexpr,iexpr) */
	case 55: /* iexpr: DIV(iexpr,iexpr) */
	case 54: /* iexpr: SUB(iexpr,iexpr) */
	case 53: /* iexpr: ADD(iexpr,iexpr) */
	case 52: /* iexpr: ATR(iassign,ilval) */
	case 48: /* ilval: INDEX(lvec,expr) */
	...
	case 1: /* finit: ARGB(bloco,params) */
		...
		break;
	case 109: /* rexpr: UMINUS(rexpr) */
	case 100: /* rexpr: PTR(rlval) */
	case 93: /* sexpr: PTR(slval) */
	case 68: /* iexpr: PTR(ilval) */
	case 67: /* iexpr: REF(lval) */
	case 66: /* iexpr: POSDEC(ilval) */
	case 65: /* iexpr: POSINC(ilval) */
	case 64: /* iexpr: DECR(ilval) */
	case 63: /* iexpr: INCR(ilval) */
	case 62: /* iexpr: UMINUS(iexpr) */
	case 61: /* iexpr: NOT(iexpr) */
	case 60: /* iexpr: FACTORIAL(iexpr) */
	case 9: /* param: PARAM(tipo,ID) */
	case 2: /* finit: ARGB(bloco,NIL) */
		kids[0] = LEFT_CHILD(p);
		break;
	case 113: /* rexpr: iexpr */
	case 106: /* rassign: rexpr */
	case 96: /* sassign: iexpr */
	case 95: /* sassign: sexpr */
	case 74: /* cmp: sexpr */
	case 73: /* cmp: iexpr */
	case 50: /* iassign: rexpr */
	case 49: /* iassign: iexpr */
	case 45: /* lval: slval */
	case 44: /* lval: rlval */
	case 43: /* lval: ilval */
	case 42: /* lvec: lval */
	...
	case 6: /* bloco: BLOCK(INSTR(list,stmt),decls) */
		...
		break;
	case 115: /* rexpr: CALL(ID,VOID) */
	case 107: /* rexpr: REAL */
	case 103: /* rlval: ID */
	case 102: /* rlval: LOCAL */
	case 99: /* sexpr: CALL(ID,VOID) */
	case 92: /* sexpr: STR */
	case 90: /* slval: ID */
	case 89: /* slval: LOCAL */
	case 72: /* iexpr: CALL(ID,VOID) */
	case 51: /* iexpr: INT */
	case 47: /* ilval: ID */
	case 46: /* ilval: LOCAL */
	case 41: /* expr: CALL(ID,VOID) */
	...
	case 8: /* decls: NONE */
		break;
	case 116: /* arguments: ARGS(NIL,expr) */
	case 114: /* rexpr: CALL(ID,arguments) */
	case 98: /* sexpr: CALL(ID,arguments) */
	case 71: /* iexpr: CALL(ID,arguments) */
	case 40: /* expr: CALL(ID,arguments) */
		kids[0] = RIGHT_CHILD(p);
		break;
	case 101: /* rexpr: PTR(PTR(ilval)) */
	case 94: /* sexpr: PTR(PTR(ilval)) */
	case 70: /* iexpr: PTR(PTR(slval)) */
	case 69: /* iexpr: PTR(PTR(ilval)) */
		kids[0] = LEFT_CHILD(LEFT_CHILD(p));
		break;
	default:
...
{fprintf(outfp, pfADDR, p->value.s);}
	...
	case 48: /* ilval: INDEX(lvec,expr) */
#line 163 "diy.brg"
{ if (isString(LEFT_CHILD(p))) fprintf(outfp, pfADD); 
